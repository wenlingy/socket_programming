...
// begin phase2  
	...
	// set back server B as UDP server
	if ((getaddrinfo("nunki.usc.edu", UDPPORTB, &hints, &server_udp)) != 0)
	{
		perror("serverB: get addr info fail\n");
		exit(1);
	}
	// else{
	// 	cout<<" Server B successfully get Back Server B addrinfo"<<endl;
	// }

	if ((server_udp_soc_num = socket(server_udp->ai_family, server_udp->ai_socktype, server_udp->ai_protocol)) == -1)
	{
		perror("serverB:creat udp soc fail\n");
		exit(1);
	}
	// else
	// {
	// 	cout<<" Back Server B successfully create socket"<<endl;
	// }
     
	if ((bind(server_udp_soc_num, server_udp->ai_addr, server_udp->ai_addrlen)) == -1)
	{
		
		perror("serverB:bind fail\n");
		exit(1);
	}
	else{
		cout<<"The Server B is up and running using UDP on port "<<UDPPORTB<<"."<<endl;
	}
	
	
	// set back server B as UDP client to send data to AWS
	if ((getaddrinfo("nunki.usc.edu", UDPPORT, &hints, &aws_addr)) != 0)
	{
		perror("serverB: get aws addr info fail\n");
		exit(1);
	}
	// else{
	// 	cout<< "Back Server B successfully get AWS addrinfo"<<endl;
	// }
	
	if ((aws_soc_num = socket(aws_addr->ai_family, aws_addr->ai_socktype, aws_addr->ai_protocol)) == -1)
	{
		perror("Back Server B:creat udp socket fail\n");
		exit(1);
	}
	// else
	// {
	// 	cout<<"Back Server B successfully set Socket for AWS addrinfo"<<endl;
	// }
	...
	while(1) 
	{
	    ...
	    if(iRec== -1)
		{
			perror(" serverB :recv nums not success\n");
			exit(1);
		}
		else
		{
			...
			cout<<"The Server B has received "<<iRec_nums<<" numbers"<<endl;
			...
			if(rec_buf.front()==1){  // max
				...
				cout<<"The Server B has successfully finished the reduction MAX:"<<*max_pos<<endl;
                ...
                {
                	perror("Server B: send result to AWS has error\n");
                	exit(1);
                }
                else{
					//cout<<"The Server B send result, numeber is: "<<iSend<<endl;
				...
				cout<<"The Server B has successfully finished the reduction MIN:"<<*min_pos<<endl;
				...
				cout<<"The Server B has successfully finished the reduction SUM:"<<sum<<endl;
				...
				cout<<"The Server B has successfully finished the reduction SOS:"<<sos<<endl;
